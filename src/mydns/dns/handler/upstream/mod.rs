...
impl UpstreamResolver {
    pub fn from_config(
        ...
        let root_hints = if root_hints.is_empty() { crate::config::default_root_hints() } else { root_hints };
        ...
        Ok(Self { cloudflare, router, mode, priority, cloudflare_addr, router_addr: effective_router, root_hints, metrics: None })
    }
...
impl DnsHandler {
    ...
    pub(crate) async fn queryUpstream(&self, name: &str, rtype: RecordType, src: SocketAddr) -> ResolutionResult {
        ...
        let result = tokio::time::timeout(Duration::from_secs(5), upstream.resolve(&parsed_name, rtype)).await;
        match result {
            ...
            Err(_) => {
                ...
                let _ = self.state.log_tx.send(format!("[UPSTREAM TIMEOUT] client={} query={} type={} server={}", src, name, rtype, addr));
