...
pub async fn persist(
    ...
    for snapshot in periods {
        ...
        .bind(
            serde_json::to_string(&snapshot.blocked_reason_counts)
                .context("serialize blocked reasons")?,
        )
        ...
        .bind(
            serde_json::to_string(&snapshot.record_type_counts)
                .context("serialize record types")?,
        )
        .bind(serde_json::to_string(&snapshot.transport_counts).context("serialize transports")?)
        .bind(
            serde_json::to_string(&snapshot.response_code_counts)
                .context("serialize response codes")?,
        )
        .bind(
            serde_json::to_string(&snapshot.resolution_outcome_counts)
                .context("serialize resolution outcomes")?,
        )
        .bind(
            serde_json::to_string(&snapshot.resolution_path_counts)
                .context("serialize resolution paths")?,
        )
        .bind(
            serde_json::to_string(&snapshot.response_latency)
                .context("serialize response latency")?,
        )
        .bind(
            serde_json::to_string(&snapshot.upstream_latency)
                .context("serialize upstream latency")?,
        )
