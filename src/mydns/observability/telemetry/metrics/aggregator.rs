...
pub struct MetricsAggregator {
    ...
    record_type_counts: Mutex<BoundedCounts>,
    transport_counts: Mutex<BoundedCounts>,
    response_code_counts: Mutex<BoundedCounts>,
    resolution_outcome_counts: Mutex<BoundedCounts>,
...
impl MetricsAggregator {
    pub fn new(timezone: Tz) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            record_type_counts: Mutex::new(BoundedCounts::default()),
            transport_counts: Mutex::new(BoundedCounts::default()),
            response_code_counts: Mutex::new(BoundedCounts::default()),
            resolution_outcome_counts: Mutex::new(BoundedCounts::default()),
            resolution_path_counts: Mutex::new(BoundedCounts::default()),
            response_latency: Mutex::new(MergeableHistogram::response()),
            upstream_latency: Mutex::new(MergeableHistogram::upstream()),
    ...
    pub fn record_upstream(&self, latency_ms: f64, success: bool, timeout: bool, retry: bool) {
        ...
        if success {
            h.current.upstream_successes += 1;
        } else {
            h.current.upstream_failures += 1;
        }
        if timeout {
            h.current.upstream_timeouts += 1;
        }
        if retry {
            h.current.upstream_retries += 1;
        }
    ...
    pub fn finalize_due_periods(&self, now: DateTime<Utc>) {
        loop {
            ...
            let snapshot = OperationalPeriodSnapshot {
                ...
                blocked_reason_counts: std::mem::take(
                    &mut *self.blocked_reason_counts.lock().unwrap(),
                ),
                ...
                response_code_counts: std::mem::take(
                    &mut *self.response_code_counts.lock().unwrap(),
                ),
                resolution_outcome_counts: std::mem::take(
                    &mut *self.resolution_outcome_counts.lock().unwrap(),
                ),
                resolution_path_counts: std::mem::take(
                    &mut *self.resolution_path_counts.lock().unwrap(),
                ),
                ...
                response_latency: std::mem::replace(
                    &mut *self.response_latency.lock().unwrap(),
                    MergeableHistogram::response(),
                ),
                upstream_latency: std::mem::replace(
                    &mut *self.upstream_latency.lock().unwrap(),
                    MergeableHistogram::upstream(),
                ),
            };
