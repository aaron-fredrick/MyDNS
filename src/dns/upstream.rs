...
fn detectGatewayImpl() -> Option<IpAddr> {
    ...
    for line in text.lines() {
        if line.contains("Default Gateway") {
            if let Some(raw) = line.split(':').nth(1) { if let Ok(ip) = raw.trim().parse::<IpAddr>() { if !ip.is_unspecified() { return Some(ip); } } }
        }
    }
    None
}
...
fn detectGatewayImpl() -> Option<IpAddr> {
    ...
    for line in content.lines().skip(1) {
        ...
        if fields.len() >= 3 && fields[1] == "00000000" { let bytes = u32::from_str_radix(fields[2], 16).ok()?; return Some(IpAddr::V4(Ipv4Addr::from(bytes.to_le_bytes()))); }
    }
    None
}
...
fn buildResolver(addr: SocketAddr) -> anyhow::Result<TokioResolver> {
    let mut udp = ConnectionConfig::udp(); udp.port = addr.port();
    let mut tcp = ConnectionConfig::tcp(); tcp.port = addr.port();
    let config = ResolverConfig::from_parts(None, vec![], vec![NameServerConfig::new(addr.ip(), true, vec![udp, tcp])]);
    let mut opts = ResolverOpts::default(); opts.timeout = Duration::from_secs(3); opts.attempts = 2;
...
pub struct UpstreamResolver {
    ...
    #[allow(dead_code)] pub cloudflare_addr: SocketAddr,
...
impl UpstreamResolver {
    #[allow(non_snake_case)]
    pub fn fromConfig(priority: ResolverPriority, cloudflare_addr: SocketAddr, router_addr: Option<SocketAddr>) -> anyhow::Result<Self> {
        ...
        if let Some(addr) = effective_router { tracing::info!(%addr, "Router/gateway DNS detected"); }
        else { tracing::warn!("Could not detect gateway DNS; router fallback unavailable"); }
        Ok(Self { cloudflare, router, priority, cloudflare_addr, router_addr: effective_router, metrics: None })
    }
    ...
    pub async fn resolve(&self, name: &Name, rtype: RecordType) -> UpstreamResolution {
        ...
        if let Some(metrics) = &self.metrics { metrics.record_upstream_start(); }
    ...
    fn orderedResolvers(&self) -> (&TokioResolver, Option<&TokioResolver>) {
        match self.priority {
            ...
            ResolverPriority::RouterFirst => if let Some(router) = &self.router { (router, Some(&self.cloudflare)) } else { (&self.cloudflare, None) },
        }
    }
}
...
async fn queryResolver(resolver: &TokioResolver, name: &Name, rtype: RecordType) -> UpstreamResolution {
    match resolver.lookup(name.clone(), rtype).await {
        ...
        Err(e) => {
            if e.is_nx_domain() { tracing::info!(query = %name, "Upstream returned NXDOMAIN"); UpstreamResolution::NxDomain }
            else if e.is_no_records_found() { tracing::info!(query = %name, "Upstream returned NODATA"); UpstreamResolution::Nodata }
            else { tracing::warn!(query = %name, error = %e, "Upstream lookup failed; returning SERVFAIL"); UpstreamResolution::ServFail }
        }
    }
}
