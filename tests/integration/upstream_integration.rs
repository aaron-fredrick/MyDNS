...
async fn start_mock_upstream() -> (SocketAddr, tokio::sync::mpsc::Receiver<Message>) {
    ...
    tokio::spawn(async move {
        ...
        while let Ok((len, src)) = socket_clone.recv_from(&mut buf).await {
            ...
            let name = msg.queries.first().unwrap().name().to_string().to_lowercase();
            if name.contains("nxdomain") {
                response.metadata.response_code = ResponseCode::NXDomain;
                let _ = socket_clone.send_to(&response.to_vec().unwrap(), src).await;
...
async fn start_dns_server(upstream_addr: SocketAddr) -> TestUpstreamServerContext {
    ...
    let cfg = AppConfig {
        bind_host: "127.0.0.1".parse().unwrap(), dns_port: port,
        http_host: "127.0.0.1".parse().unwrap(), http_port: port + 1,
        cors_domains: vec!["mydns.local".to_string()], dashboard_domain: "mydns.local".to_string(),
        db_path: db.path_str(), jwt_secret: mydns::config::generate_secret(64),
        admin_username: "admin".to_string(), admin_password: "changeme123".to_string(),
        resolver_mode: mydns::config::ResolverMode::Forwarding,
        resolver_priority: mydns::config::ResolverPriority::CloudflareFirst,
        cloudflare_dns: upstream_addr, router_dns: Some(upstream_addr),
        run_as_user: "nobody".to_string(), run_as_group: "nobody".to_string(),
        allowed_zones: vec![], root_hints: vec![],
    };
    let pool = db.init_pool().await;
    let hash = mydns::web::auth::hash_password(&cfg.admin_password).unwrap();
    db::records::seed_admin(&pool, &cfg.admin_username, &hash).await.unwrap();
    let upstream = dns::upstream::UpstreamResolver::from_config(
        cfg.resolver_mode.clone(), cfg.resolver_priority.clone(), cfg.cloudflare_dns,
        cfg.router_dns, cfg.root_hints.clone(),
    ).unwrap();
    let (log_tx, _) = tokio::sync::broadcast::channel(1024);
    let cancel = CancellationToken::new();
    let zone_trie = ZoneTrie::from_zones(&cfg.allowed_zones);
    let record_index = RecordIndex::load_from_db(&pool).await.unwrap();
    let domains = mydns::db::blocklist::list_enabled_domains(&pool).await.unwrap();
    let blocklist_index = mydns::dns::blocklist::BlocklistIndex::from_domains(&domains);
    let state = AppState::new(pool, cfg, upstream, log_tx, cancel.clone(), record_index, zone_trie, blocklist_index);
    let server_state = Arc::clone(&state);
    let server_cancel = cancel.clone();
    let task = tokio::spawn(async move { let _ = dns::server::run(server_state, server_cancel).await; });
    for _ in 0..50 {
        if tokio::net::TcpStream::connect(addr).await.is_ok() { break; }
        tokio::time::sleep(Duration::from_millis(20)).await;
    }
    TestUpstreamServerContext { db, addr, cancel, task }
}
...
fn query_message(name: &str) -> Vec<u8> {
    ...
    let mut bytes = message.to_vec().unwrap(); bytes[2] |= 0b0000_0001; bytes
}

async fn udp_query(socket: &UdpSocket, addr: SocketAddr, name: &str) -> Message {
    socket.send_to(&query_message(name), addr).await.unwrap();
    let mut buf = [0u8; 4096];
    let (len, _) = tokio::time::timeout(Duration::from_secs(15), socket.recv_from(&mut buf)).await.unwrap().unwrap();
