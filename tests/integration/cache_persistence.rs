...
async fn test_positive_cache_survives_pool_restart() {
...

    {
        ...
        db::cache::insert_cache(&pool, "restart.test.local.", "A", "10.1.2.3", 300, None)
            .await
            .unwrap();
        pool.close().await;
    }
    ...
    let pool = test_db.init_pool().await;
    let rows = db::cache::get_cache(&pool, "restart.test.local.", "A")
        .await
        .unwrap();
...
async fn test_negative_cache_survives_pool_restart() {
    ...
    {
        let pool = test_db.init_pool().await;
        db::cache::insert_cache(&pool, "missing.restart.test.local.", "A", "NX", 300, None)
            .await
            .unwrap();
        pool.close().await;
    }
    ...
    let pool = test_db.init_pool().await;
    let rows = db::cache::get_cache(&pool, "missing.restart.test.local.", "A")
        .await
        .unwrap();
...
async fn test_expired_persistent_cache_is_hidden_and_pruned() {
    ...
    db::cache::insert_cache(&pool, "expired.test.local.", "A", "10.9.8.7", 0, None)
        .await
        .unwrap();

    let visible = db::cache::get_cache(&pool, "expired.test.local.", "A")
        .await
        .unwrap();
    assert!(visible.is_empty(), "Expired entries must not be returned");

    let pruned = db::cache::prune_cache(&pool).await.unwrap();
    assert_eq!(pruned, 1, "Expired entry should be physically pruned");

    let all = db::cache::list_cache_entries(&pool).await.unwrap();
    assert!(all.is_empty());
}
...
async fn test_persistent_cache_clear_removes_all_entries() {
    ...
    db::cache::insert_cache(&pool, "one.clear.test.local.", "A", "10.0.0.1", 300, None)
        .await
        .unwrap();
    db::cache::insert_cache(&pool, "two.clear.test.local.", "A", "10.0.0.2", 300, None)
        .await
        .unwrap();
    ...
    assert!(db::cache::list_cache_entries(&pool)
        .await
        .unwrap()
        .is_empty());
}
...
async fn test_concurrent_cache_upserts_remain_deduplicated() {
    ...
    let rows = db::cache::get_cache(&pool, "concurrent.test.local.", "A")
        .await
        .unwrap();
    assert_eq!(
        rows.len(),
