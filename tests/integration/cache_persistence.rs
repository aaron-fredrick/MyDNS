...
async fn test_positive_cache_survives_pool_restart() {
    ...
    let pool = test_db.init_pool().await;
    let rows = db::records::get_cache(&pool, "restart.test.local.", "A").await.unwrap();
...
async fn test_negative_cache_survives_pool_restart() {
    ...
    let pool = test_db.init_pool().await;
    let rows = db::records::get_cache(&pool, "missing.restart.test.local.", "A").await.unwrap();
    assert_eq!(rows.len(), 1);
...
async fn test_expired_persistent_cache_is_hidden_and_pruned() {
    ...
    db::records::insert_cache(&pool, "expired.test.local.", "A", "10.9.8.7", 0, None)
        .await
        .unwrap();
    let visible = db::records::get_cache(&pool, "expired.test.local.", "A").await.unwrap();
    assert!(visible.is_empty());
    let pruned = db::records::prune_cache(&pool).await.unwrap();
    assert_eq!(pruned, 1);
    assert!(db::records::list_cache_entries(&pool).await.unwrap().is_empty());
}
...
async fn test_persistent_cache_clear_removes_all_entries() {
    ...
    db::records::insert_cache(&pool, "two.clear.test.local.", "A", "10.0.0.2", 300, None)
        .await
        .unwrap();
    assert_eq!(db::records::list_cache_entries(&pool).await.unwrap().len(), 2);
...
async fn test_concurrent_cache_upserts_remain_deduplicated() {
    ...
    for ttl in 300..=331u32 {
        ...
        tasks.push(tokio::spawn(async move {
            db::records::insert_cache(&pool, "concurrent.test.local.", "A", "10.20.30.40", ttl, None)
                .await
        }));
    }
    for task in tasks {
    ...
    let rows = db::records::get_cache(&pool, "concurrent.test.local.", "A").await.unwrap();
    assert_eq!(rows.len(), 1);
}
